#' @title vote_info_XML2RDS
#'
#' @description Transform vote details list from XML files to RDS files
#'
#' @usage vote_info_XML2RDS(XML_WD = "",
#'                   RDS_WD = "")
#'
...
vote_info_XML2RDS = function(XML_WD, RDS_WD){
  ...
  for(i in 1:length(vote_list_files)){
    ...
    setwd(XML_WD)
    vote_df = vote_info_cleaning(xml_database = read_xml(paste0(vote_list_files[i])))
    setwd(RDS_WD)
